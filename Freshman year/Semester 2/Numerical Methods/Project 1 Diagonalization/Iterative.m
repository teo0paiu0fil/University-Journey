function R = Iterative(nume, d, eps)
  ...
  vector1 = ones(nr,1);
  % pastrez intr-o variabila Valoare anterioara a lui R
  RAnt = R + 1;
  
  % folosesc formula de la sursa pana cand se ajunge la precizia dorita
  while norm(R - RAnt) > eps
    RAnt = R; 
    R = d * MatAdiacenta* R + ( (1 - d) / nr) * vector1;
  endwhile
  % oentru a respecta t din formula din prima sursa vom transimte RAnt;
    R = RAnt;
  endfunction
