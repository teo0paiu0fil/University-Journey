...
attach t1 EmptyTree = t1
attach EmptyTree t2 = t2
attach tree1@(Node p1 c1 cxs) tree2@(Node p2 c2 cxl) 
    | p1 < p2 = Node p1 c1 $ [tree2] ++ cxs 
    | p2 < p1 = Node p2 c2 $ [tree1] ++ cxl
    | p2 == p1 = Node p1 c1 $ tree2:cxs 
...
insertTree tr1 (tr2:trees) = let 
    calculateRank (Node p c childrens) = length childrens
    calculateRank EmptyTree = 0
    isEmptyTree EmptyTree = True
    isEmptyTree (Node _ _ _) = False
    in case (calculateRank tr1) == (calculateRank tr2) of
        True -> if (isEmptyTree tr2) then
                tr1:trees
            else 
               EmptyTree:(insertTree (attach tr1 tr2) trees)
        False -> if (isEmptyTree tr2) then
                tr1:trees
            else 
                tr2:(insertTree tr1 trees)
...
insert prio key heap@(BinomialHeap p k) = BinomialHeap (p + 1)  (insertTree (Node prio key []) k)
...
findMin (BinomialHeap _ k) = case minimumBy (compare `on` fst) [(p', k') | (Node p' k' _) <- k] of
    (p', k') -> Just (p', k')
...
mergeTrees ts1 ts2 = (snd $ mapAccumL mergeAux EmptyTree $ zipExtend EmptyTree EmptyTree ts1 ts2 )  ++ [fst $ mapAccumL mergeAux EmptyTree $ zipExtend EmptyTree EmptyTree ts1 ts2 ]
    where 
        mergeAux carry@(Node _ _ _) (EmptyTree, EmptyTree) = (EmptyTree, carry)
        mergeAux carry@(Node _ _ _) (EmptyTree, tr2@(Node _ _ _)) = (attach tr2 carry, EmptyTree)
        mergeAux carry@(Node _ _ _) (tr1@(Node _ _ _), EmptyTree) = (attach tr1 carry, EmptyTree)
        mergeAux carry@(Node _ _ _) (tr1@(Node _ _ _), tr2@(Node _ _ _)) = (attach tr1 tr2, carry)
        mergeAux EmptyTree (EmptyTree, EmptyTree) = (EmptyTree, EmptyTree)
        mergeAux EmptyTree (EmptyTree, tr2@(Node _ _ _)) = (EmptyTree, tr2)
        mergeAux EmptyTree (tr1@(Node _ _ _), EmptyTree) = (EmptyTree, tr1)
        mergeAux EmptyTree (tr1@(Node _ _ _), tr2@(Node _ _ _)) = (attach tr1 tr2, EmptyTree)
...
merge (BinomialHeap 0 []) heap2@(BinomialHeap x2 trees2) = heap2
merge heap1@(BinomialHeap x1 trees1) (BinomialHeap 0 []) = heap1
merge heap1@(BinomialHeap x1 trees1) heap2@(BinomialHeap x2 trees2) = BinomialHeap (x1 + x2) $ mergeTrees trees1 trees2