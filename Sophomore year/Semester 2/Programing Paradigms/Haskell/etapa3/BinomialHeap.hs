...
-}
isolate :: a -> [a] -> [(a, [a])]
isolate placeHolder list = deleteLastPlacehoder $ zip list (isolatedLists placeHolder list) 
    where
        isolatedLists :: a -> [a] -> [[a]]
        isolatedLists _ [] = []
        isolatedLists y (z:zs) = [y : zs] ++ map (z:) (isolatedLists y zs)
        deleteLastPlacehoder [] = []
        deleteLastPlacehoder [(b, xs)] = [(b, take (length xs - 1) xs)]
        deleteLastPlacehoder (p:ps) = p : deleteLastPlacehoder ps
...
removeMin (BinomialHeap p k) = (BinomialHeap (p - 1) (auxF k))
    where 
        auxF k = case (fst $ rez k) of
          (Node p c []) ->  (snd $ rez k)
          (Node p c (x:ks)) ->  insertALL (x:ks) (snd $ rez k)
        insertALL (x:ks) l = insertALL (ks) (insertTree x l) 
        insertALL [] l = l
        rez k = minimumBy (compare `on` (\ ((Node p k _), _ ) -> p)) [ (nod, k) | (nod@(Node p _ _), k)  <- (isolate EmptyTree k)]
{-
    *** TODO ***
...
instance (Show p, Show k) => Show (BinomialTree p k) where
    ...
    show tree = let 
          deleteLastPlacehoder [] = []
          deleteLastPlacehoder [(b)] = []
          deleteLastPlacehoder (p:ps) = p : deleteLastPlacehoder ps
          showTree level (Node p k xl) = replicate level ' ' ++ show p ++ " (" ++ show k ++ ")" ++ "\n" ++ (concatMap (showTree (level + 2)) xl)
        in deleteLastPlacehoder $ showTree 0 tree
...
instance (Show p, Show k) => Show (BinomialHeap p k) where
    show (BinomialHeap 0 _) =  ""
    show (BinomialHeap p xl) = intercalate ['\n'] (map show xl)
...
instance Functor (BinomialTree p) where
    ...
    fmap f (Node p k x) = Node p (f k) (map auxF x)
        where
          auxF EmptyTree = EmptyTree
          auxF (Node p k xl) = Node p (f k) (map auxF xl)
{-
    *** TODO ***
...
instance Functor (BinomialHeap p) where
    ...
    fmap f (BinomialHeap 0 _) = (BinomialHeap 0 [])
    fmap f (BinomialHeap p k) = (BinomialHeap p (map auxF k))
        where
          auxF EmptyTree = EmptyTree
          auxF (Node p k xl) = Node p (f k) (map auxF xl)
{-
...
instance Foldable (BinomialTree p) where
    ...
    foldr f acc (Node _ k l) = f k (foldr (flip $ foldr f) acc l)