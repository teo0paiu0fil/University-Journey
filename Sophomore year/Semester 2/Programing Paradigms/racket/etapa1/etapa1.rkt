...
(define (get-men mpref)
  (if (null? mpref) '() (append (list (caar mpref)) (get-men (cdr mpref))))
  )
...
(define (get-women wpref)
  (get-women-coada null wpref))

(define (get-women-coada n wpref)
 (if (null? wpref) n (get-women-coada (append n (list (caar wpref))) (cdr wpref)))
  )
...
(define (get-pref-list pref person)
 (if (equal? (caar pref) person) (cdr (car pref)) (get-pref-list (cdr pref) person))
  )
...
(define (preferable? pref-list x y)
  (cond ((equal? y (car pref-list)) (and false (preferable? (cdr pref-list) x y)))
        ((equal? x (car pref-list)) true)
...
(define (get-partner engagements person)
  (cond ((null? engagements) false)
        ((equal? person (car (car engagements))) (cdr (car engagements)))
