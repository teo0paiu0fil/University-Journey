...
(define (get-women wpref)
  (foldr add-first-element '() wpref))

(define (add-first-element L acc)
  (cons (car L) acc))
...
(define (get-pref-list pref person)
  (foldr (lambda (x acc) (if (equal? person (car x)) (append acc (cdr x)) acc)) '() pref))
...
(define (preferable? pref-list x y)
  (if (equal? (car (filter (lambda (z) (or (equal? x z) (equal? y z))) pref-list)) x) #t #f))
...
(define (get-partner engagements person)
  (define parteners? (find-first (lambda (x) (equal? (car x) person)) engagements))
  (if (equal? parteners? #f) #f (cdr parteners?))
  )
...
(define (change-first p L val)
  (cond ((null? L) '())
        ((p (car L)) (cons val (change-first (lambda (x) #f) (cdr L) val)))
...
(define (better-match-exists? p1 p2 p1-list pref2 engagements)
  (cond ((null? p1-list) false)
        ((equal? p2 (car p1-list)) false)
        ((preferable? p1-list (car p1-list) p2) (if (preferable? (get-pref-list pref2 (car p1-list)) p1 (get-partner engagements (car p1-list))) true  (better-match-exists? p1 p2 (cdr p1-list) pref2 engagements)))
...
(define (stable-match? engagements mpref wpref)
  (if (member #t (map (lambda (x) (or (better-match-exists? (car x) (cdr x) (get-pref-list wpref (car x)) mpref (map reverse-pair engagements))
                        (better-match-exists? (cdr x) (car x) (get-pref-list mpref (cdr x)) wpref engagements))) engagements)) #f #t)
  )
